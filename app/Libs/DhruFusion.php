...
class DhruFusion
{
    ...
    var $debug;
    var $action;
    // custom by merma158 at Innodite Inc
    var $credential_user;
    var $credential_api_key;
    var $credential_request_format;
    var $credential_base_uri;
    var $log_response;
    // end custom block
    function __construct($credentials)
    {
        ...
        // custom by merma158 at Innodite Inc
        $this->credential_base_uri       = $credentials["DHRUFUSION_URL"];
        $this->credential_user           = $credentials["USERNAME"];
        $this->credential_api_key        = $credentials["API_ACCESS_KEY"];
        $this->credential_request_format = $credentials["REQUESTFORMAT"];
        // end custom block
    }
    ...
    function action($action, $arr = array())
    {
        if (is_string($action))
        {
            if (is_array($arr))
            {
                ...
                $posted = array(
                    'username' => $this->credential_user,
                    'apiaccesskey' => $this->credential_api_key,
                    'action' => $action,
                    'requestformat' => $this->credential_request_format,
                ...
                curl_setopt($crul, CURLOPT_URL, $this->credential_base_uri.'/api/index.php');
